...
import {
  getReportBlock,
  getReportList,
} from "../../Redux/ReportManagement/actions";
import {
  getDisputeList,
  getDisputeBlock,
} from "../../Redux/DisputeManagement/actions";
...
const DisputeManagement = (item) => {
  ...
  const DisputeList = useSelector(
    (state) => state.DisputeListing?.Reports?.disputedJobListing
  );
  const DisputePagination = useSelector(
    (state) => state.DisputeListing?.Reports
  );
    ...
    return (
      <tr>
        ...
        <td style={{ paddingLeft: "2%" }}>
          <span className="fw-normal">
            {DisputeList?.description ? DisputeList?.description : "N/A"}
          </span>
        </td>
        <td>
          <span className="fw-normal">
        ...
        <td>
          <span className="fw-normal">
            {item?.disputedTo?.fullName ? item?.disputedTo?.fullName : "N/A"}
          </span>
        </td>
          ...
          <Table hover className="user-table align-items-center">
            ...
            <tbody>
              {dataList?.map((obj, index) => {
                return <TableRow index={index} item={obj} />;
              })}
            </tbody>
          </Table>
